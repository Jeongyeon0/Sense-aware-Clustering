# Sense-aware Clustering algorithm
...
def get_homonym_list(target_list):
    word_list = []
...
def is_common_homonym(cluster1, cluster2):
    cluster1 = set(get_homonym_list(cluster1))
    cluster2 = set(get_homonym_list(cluster2))
...
def main():
    ...
    while (1):
        ...
        # Clustering steps
        distance, I = index.search(centroid_vec_dict[query_key].reshape(1, -1), k=arg.num_k+1)
        ...
        for d, i in zip(distance[0], I[0]):
            ...
            if cosine_value >= threshold:
                if (is_common_homonym(cls_dict[query_key], cls_dict[i]) == False) or (cosine_value==1.0):
                    cls_dict, centroid_vec_dict = clustering_and_get_centroid(cls_dict, centroid_vec_dict, [query_key, i])
                    index.remove_ids(np.array([i]))
                    query_cand = list(cls_dict.keys())
                    
                elif (cosine_value >= threshold) and (is_common_homonym(cls_dict[query_key], cls_dict[i]) == True):
...
# HaSC.py 파일에 같은 코드를 복사하여 만들었음. 이 코드는 백업용임
