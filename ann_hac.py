...
def main():
    ...
    parser.add_argument("--output_file", type=str, default="test_result.txt")
    ...
    print("Parameter setting\n nbit: {0}, threshold: {1}, num_k: {2}, mode: {3}\n input_file: {4}, output_file: {5} ".format(arg.nbit, arg.threshold, arg.num_k, arg.mode, arg.vector_file, arg.output_file))
    ...
    while(1):
        ...
        for d, i in zip(distance[0], I[0]):
            ...
            #print(cosine_value)
            if (cosine_value >= threshold) and (isHomonym(cls_dict[query_key], cls_dict[i])==False):
        ...
        index.add_with_ids(np.array(centroid_vec_dict[query_key]).reshape(1,-1).astype('float32'), np.array([query_key]).astype('int64'))
        #print('now cls_dict: {0}, now flag: {1}'.format(cls_dict.keys(),flag))
        if cnt%500==0:
            dict_size = len(cls_dict.keys())
            print('now step: {0}, clustering state: {1}/{2}, compression_rate: {3: .3f}%'.format(cnt, dict_size, total_len, (1-(dict_size/total_len))*100))
        cnt+=1
    ...
    for key in cls_dict.keys():
        save.write(str(key))
        for i in range(len(cls_dict[key])):
            save.write(' '+cls_dict[key][i])
        save.write('\n')
